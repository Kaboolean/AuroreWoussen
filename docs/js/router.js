...
class Router {
    constructor() {
        ...
        // Initialiser la langue depuis localStorage (défaut: 'fr')
        this.lang = localStorage.getItem('lang') || 'fr';
        console.log('Router initialisé avec basePath:', this.basePath, '| langue:', this.lang);
    }
    ...
    init() {
        ...
        // Appliquer la langue au header/footer
        this.applyLanguage();
    ...
    async loadPage(page) {
        ...
        let templatePath = this.routes[page] || this.routes['home'];
        
        // Si la langue est anglais, charger depuis pages/en/
        if (this.lang === 'en') {
            templatePath = templatePath.replace('pages/', 'pages/en/');
        }
    ...
    /**
     * Initialise les boutons de sélection de langue
     */
    initLanguageToggle() {
        // Sélectionner tous les boutons de langue (desktop et mobile)
        document.addEventListener('click', (e) => {
    ...
    setLang(lang) {
        ...
        // Recharger la page courante avec la nouvelle langue
        this.currentPage = null; // Force le rechargement
        this.loadPage(this.currentPage || 'home');
        this.handleRoute();
    }
    ...
    updateLanguageButtons() {
        ...
        document.querySelectorAll('.lang-toggle__btn').forEach(btn => {
            btn.classList.toggle('lang-toggle__btn--active', btn.getAttribute('data-lang') === this.lang);
        });
        
        // Boutons mobile
        document.querySelectorAll('.lang-toggle-mobile__btn').forEach(btn => {
            btn.classList.toggle('lang-toggle-mobile__btn--active', btn.getAttribute('data-lang') === this.lang);
        });
    }

    /**
     * Applique la langue au header et footer (éléments statiques)
     */
    applyLanguage() {
        // Mettre à jour les boutons de langue
        this.updateLanguageButtons();
        ...
        document.documentElement.lang = this.lang === 'en' ? 'en' : 'fr';
        
        // Traductions du header/footer
        const translations = {
            fr: {
                subtitle: 'CHIRURGIE PLASTIQUE ET RECONSTRUCTRICE',
                nav: {
                    home: 'Accueil',
                    presentation: 'Dr Aurore Woussen',
                    articles: 'Articles',
                    surgery: 'Chirurgie',
                    surgeryPath: 'Parcours de chirurgie',
                    breastSurgery: 'Chirurgie du sein',
                    bodySurgery: 'Chirurgie de la silhouette',
                    faceSurgery: 'Chirurgie du visage',
                    beforeAfter: 'Avant / après',
                    prices: 'Tarifs',
                    office: 'Mon cabinet',
                    // Sous-menus sein
                    breastImplants: 'Prothèses mammaires',
                    breastReduction: 'Réduction mammaire',
                    breastLift: 'Lifting mammaire',
                    breastLipofilling: 'Lipofilling mammaire',
                    invertedNipples: 'Mamelons ombiliqués',
                    gynecomastia: 'Gynécomastie',
                    // Sous-menus silhouette
                    liposuction: 'Lipoaspiration',
                    tummyTuck: 'Abdominoplastie',
                    bodyLift: 'Body lift',
                    mommyMakeover: 'Mommy Make over',
                    armLift: 'Brachioplastie',
                    thighLift: 'Cruroplastie',
                    // Sous-menus visage
            ...
            en: {
                subtitle: 'PLASTIC AND RECONSTRUCTIVE SURGERY',
                nav: {
                    home: 'Home',
                    presentation: 'Dr Aurore Woussen',
                    articles: 'Articles',
                    surgery: 'Surgery',
                    surgeryPath: 'Surgery journey',
                    ...
                    office: 'My office',
                    // Sous-menus sein
                    breastImplants: 'Breast implants',
                    breastReduction: 'Breast reduction',
                    breastLift: 'Breast lift',
                    breastLipofilling: 'Breast lipofilling',
                    invertedNipples: 'Inverted nipples',
        ...
        const navMappings = [
            { selector: '[data-page="home"]', text: t.nav.home },
            { selector: '[data-page="presentation"]', text: t.nav.presentation },
            { selector: '[data-page="articles"]', text: t.nav.articles },
            { selector: '[data-page="avant-apres"]', text: t.nav.beforeAfter },
            { selector: '[data-page="tarifs"]', text: t.nav.prices },
            { selector: '[data-page="mon-cabinet"]', text: t.nav.office },
            { selector: '[data-page="parcours-chirurgie"]', text: t.nav.surgeryPath },
            // Chirurgie du sein
            { selector: '[data-page="protheses-mammaires"]', text: t.nav.breastImplants },
            { selector: '[data-page="reduction-mammaire"]', text: t.nav.breastReduction },
            { selector: '[data-page="lifting-mammaire"]', text: t.nav.breastLift },
            { selector: '[data-page="lipofilling-mammaire"]', text: t.nav.breastLipofilling },
            { selector: '[data-page="mamelons-ombiliques"]', text: t.nav.invertedNipples },
            { selector: '[data-page="gynecomastie"]', text: t.nav.gynecomastia },
            // Chirurgie de la silhouette
            { selector: '[data-page="lipoaspiration"]', text: t.nav.liposuction },
            { selector: '[data-page="abdominoplastie"]', text: t.nav.tummyTuck },
            { selector: '[data-page="body-lift"]', text: t.nav.bodyLift },
            { selector: '[data-page="mommy-makeover"]', text: t.nav.mommyMakeover },
            { selector: '[data-page="brachioplastie"]', text: t.nav.armLift },
            { selector: '[data-page="cruroplastie"]', text: t.nav.thighLift },
            // Chirurgie du visage
            { selector: '[data-page="lifting-cervico-facial"]', text: t.nav.facelift },
            { selector: '[data-page="blepharoplastie"]', text: t.nav.eyelidSurgery },
            { selector: '[data-page="oreilles-decollees"]', text: t.nav.earSurgery },
            { selector: '[data-page="mentions-legales"]', text: t.nav.legalNotice }
        ];
        
        navMappings.forEach(({ selector, text }) => {
            document.querySelectorAll(selector).forEach(el => {
                // Ne pas modifier le logo du header
                if (el.classList.contains('header__logo')) {
                    return;
                }
                // Préserver les icônes/flèches dans les liens
        ...
        // Mettre à jour les titres des dropdowns (Chirurgie, Chirurgie du sein, etc.)
        const dropdownLinks = document.querySelectorAll('.nav__item--dropdown > .nav__link');
        dropdownLinks.forEach(link => {
            const arrow = link.querySelector('.dropdown-arrow');
            if (arrow) {
                link.childNodes[0].textContent = t.nav.surgery + ' ';
            }
        });
        
        // Mettre à jour les sous-catégories
        document.querySelectorAll('.nav__dropdown-item--has-submenu > .nav__dropdown-link').forEach(link => {
            const text = link.textContent.trim();
            const arrow = link.querySelector('.dropdown-arrow');
            if (text.includes('sein') || text.includes('Breast')) {
                link.childNodes[0].textContent = t.nav.breastSurgery + ' ';
            } else if (text.includes('silhouette') || text.includes('Body')) {
                link.childNodes[0].textContent = t.nav.bodySurgery + ' ';
            } else if (text.includes('visage') || text.includes('Facial')) {
                link.childNodes[0].textContent = t.nav.faceSurgery + ' ';
            }
        });
        
        // Mettre à jour le copyright du footer
    ...
    getPageTitle(page) {
        const titles = {
            ...
            en: {
                'protheses-mammaires': 'Breast implants',
                'reduction-mammaire': 'Breast reduction',
                'lifting-mammaire': 'Breast lift',
                'lipofilling-mammaire': 'Breast lipofilling',
                'mamelons-ombiliques': 'Inverted nipples',
                'gynecomastie': 'Gynecomastia',
                'lipoaspiration': 'Liposuction',
                'abdominoplastie': 'Tummy tuck',
                'body-lift': 'Body lift',
                'mommy-makeover': 'Mommy Makeover',
                'brachioplastie': 'Arm lift',
                'cruroplastie': 'Thigh lift',
                'lifting-cervico-facial': 'Facelift',
                'blepharoplastie': 'Eyelid surgery',
                'oreilles-decollees': 'Ear surgery'
            }
        };
        return titles[this.lang]?.[page] || titles['fr'][page] || page;
    }
}
